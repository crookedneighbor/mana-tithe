...
describe('Results', () => {
	it('creates a result for each card row', () => {
		...
		expect(screen.getByTestId('result-row-0-name').querySelector('a').href).toBe(
			'https://example.com/scryfall/card-a'
		);
		expect(screen.getByTestId('result-row-1-name')).toHaveTextContent('Card B');
		expect(screen.getByTestId('result-row-1-name').querySelector('a').href).toBe(
			'https://example.com/scryfall/card-b'
		);
		expect(screen.getByTestId('result-row-2-name')).toHaveTextContent('Card C');
		expect(screen.getByTestId('result-row-2-name').querySelector('a').href).toBe(
			'https://example.com/scryfall/card-c'
		);

		expect(screen.getByTestId('result-row-0-price')).toHaveTextContent('$123.12');
		expect(screen.getByTestId('result-row-0-price').querySelector('a').href).toBe(
			'https://example.com/tcg/card-a'
		);
		expect(screen.getByTestId('result-row-1-price')).toHaveTextContent('$913.00');
		expect(screen.getByTestId('result-row-1-price').querySelector('a').href).toBe(
			'https://example.com/tcg/card-b'
		);
		expect(screen.getByTestId('result-row-2-price')).toHaveTextContent('$151.53');
		expect(screen.getByTestId('result-row-2-price').querySelector('a').href).toBe(
			'https://example.com/tcg/card-c'
		);
	});
	...
	it('calculates the total price of the deck', () => {
		render(Results, {
			props: {
				results: [createCardRow('Card A', 1, 123.12), createCardRow('Card B', 1, 456.5)]
			}
		});
	...
	it('accounts for qty in price calculation', () => {
		render(Results, {
			props: {
				results: [createCardRow('Card A', 2, 123.12), createCardRow('Card B', 1, 456.5)]
			}
		});
	...
	it('does not render tcg player link if not provided', () => {
		render(Results, {
			props: {
				results: [createCardRow('Card A', 2, 0), createCardRow('Card B', 1, 456.5)]
			}
		});
